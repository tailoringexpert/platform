...
import static de.baedorf.tailoringexpert.domain.MediaTypeProvider.ATTACHMENT;
import static de.baedorf.tailoringexpert.domain.MediaTypeProvider.FORM_DATA;
import static de.baedorf.tailoringexpert.domain.ResourceMapper.KATALOG;
import static de.baedorf.tailoringexpert.domain.ResourceMapper.KATALOGVERSION;
import static de.baedorf.tailoringexpert.domain.ResourceMapper.KATALOGVERSIONJSONDOWNLOAD;
import static de.baedorf.tailoringexpert.domain.ResourceMapper.KATALOGVERSIONPDFDOWNLOAD;
...
public class KatalogController {
    ...
    @PostMapping(value = KATALOG, produces = {"application/hal+json"})
    @SneakyThrows
    ...
    @GetMapping(value = KATALOG, produces = {"application/hal+json"})
    ...
    @GetMapping(value = KATALOGVERSION, produces = {"application/json"})
    ...
    @GetMapping(KATALOGVERSIONPDFDOWNLOAD)
    @ResponseBody
    public ResponseEntity<byte[]> getOutputKatalog(@Parameter(description = "Version des angefragten Katalogs") @PathVariable("version") String version) {
        return katalogService.createKatalog(version)
            .map(dokument -> ResponseEntity
                .ok()
                .header(CONTENT_DISPOSITION, ContentDisposition.builder(FORM_DATA).name(ATTACHMENT).filename(dokument.getName()).build().toString())
    ...
    @GetMapping(KATALOGVERSIONJSONDOWNLOAD)
    @ResponseBody
    public ResponseEntity<byte[]> getJsonKatalog(@Parameter(description = "Version des angefragten Katalogs") @PathVariable("version") String version) throws JsonProcessingException {
        ...
        return ResponseEntity
            .ok()
            .header(CONTENT_DISPOSITION, ContentDisposition.builder(FORM_DATA).name(ATTACHMENT).filename("katalog_v" + version + ".json").build().toString())
            .header(ACCESS_CONTROL_EXPOSE_HEADERS, CONTENT_DISPOSITION)
