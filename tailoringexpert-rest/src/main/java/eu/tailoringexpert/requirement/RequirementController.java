...
import static eu.tailoringexpert.domain.ResourceMapper.TAILORINGREQUIRMENT_CHANGES;
...
public class RequirementController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Requirement geladen", content = @Content(mediaType = "application/json+hal", schema = @Schema(implementation = TailoringRequirementResource.class))),
            @ApiResponse(responseCode = "404", description = "Requirement nicht vorhanden", content = @Content)
    })
    ...
    public ResponseEntity<EntityModel<TailoringRequirementResource>> getRequirement(
        ...
        PathContextBuilder pathContext = PathContext.builder()
                ...
                .tailoring(tailoring)
                .chapter(chapter);
        ResponseEntity<EntityModel<TailoringRequirementResource>> result = requirementServiceRepository
                .getRequirement(project, tailoring, chapter, requirement)
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "Requirement does not exist", content = @Content)
    })
    ...
    public ResponseEntity<EntityModel<TailoringRequirementResource>> putRequirementState(
        ...
        ResponseEntity<EntityModel<TailoringRequirementResource>> result = requirementService
                .handleSelected(project, tailoring, chapter, requirement, selected)
                .map(projektPhaseAnforderung -> ResponseEntity
                        .ok()
                        .body(EntityModel.of(mapper.toResource(pathContext, projektPhaseAnforderung))))
                .orElseGet(() -> notFound().build());
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Text changed"),
            @ApiResponse(responseCode = "404", description = "Requirement does not exist", content = @Content)
    })
    ...
    public ResponseEntity<EntityModel<TailoringRequirementResource>> putRequirementText(
        ...
        ResponseEntity<EntityModel<TailoringRequirementResource>> result = requirementService
                .handleText(project, tailoring, chapter, requirement, text)
                .map(projektPhaseAnforderung -> ResponseEntity
                        .ok()
                        .body(EntityModel.of(mapper.toResource(pathContext, projektPhaseAnforderung))))
                .orElseGet(() -> notFound().build());
    ...
    public ResponseEntity<EntityModel<TailoringCatalogChapterResource>> putRequirementsState(
        ...
        ResponseEntity<EntityModel<TailoringCatalogChapterResource>> result = requirementService
                .handleSelected(project, tailoring, chapter, selected)
    ...
    @Operation(summary = "load list of requirement changes")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "changes loaded", content = @Content(mediaType = "application/json+hal", schema = @Schema(implementation = TailoringRequirementResource.class))),
            @ApiResponse(responseCode = "404", description = "Requirement does not exists", content = @Content)
    })
    @GetMapping(value = TAILORINGREQUIRMENT_CHANGES, produces = { "application/hal+json" })
    public ResponseEntity<CollectionModel<EntityModel<RequirementChangeResource>>> getRequirementChanges(
            @Parameter(description = "Project identifier") @PathVariable String project,
            @Parameter(description = "Tailoring name") @PathVariable String tailoring,
            @Parameter(description = "Chapter number") @PathVariable String chapter,
            @Parameter(description = "Requirement position in chapter") @PathVariable String requirement) {
        ...
        Optional<Collection<RequirementChange>> requirementChanges = requirementService.getRequirementChanges(project,
                tailoring, chapter, requirement);

        if (requirementChanges.isEmpty()) {
            log.traceExit();
            return notFound().build();
        }

        ResponseEntity<CollectionModel<EntityModel<RequirementChangeResource>>> result = ok()
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Requirement added to tailoring", content = @Content(mediaType = "application/json+hal", schema = @Schema(implementation = TailoringRequirementResource.class))),
    ...
    public ResponseEntity<EntityModel<TailoringRequirementResource>> postRequirement(
        ...
        PathContextBuilder pathContext = PathContext.builder()
                ...
                .chapter(chapter);
        ResponseEntity<EntityModel<TailoringRequirementResource>> result = requirementService
                .createRequirement(project, tailoring, chapter, requirement, text)
