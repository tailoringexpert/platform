...
import eu.tailoringexpert.domain.ProjektInformationResource;
import eu.tailoringexpert.domain.ProjektResource;
...
public class ProjektController {
    ...
    public ResponseEntity<Void> createProjekt(
        ...
        return ResponseEntity
            .created(mapper.createLink(ResourceMapper.REL_SELF, linkToCurrentMapping().toString(), ResourceMapper.PROJEKT, Map.of("projekt", projekt.getProjekt())).toUri())
            .build();
    ...
    @GetMapping(value = ResourceMapper.PROJEKT, produces = {"application/hal+json"})
    ...
    @GetMapping(ResourceMapper.PROJEKTSCREENINGSHEETDATEI)
    @ResponseBody
    ...
    @DeleteMapping(value = ResourceMapper.PROJEKT, produces = {"application/hal+json"})
    ...
    @PostMapping(value = ResourceMapper.PROJEKT, produces = {"application/hal+json"})
    @ResponseBody
    @SneakyThrows
    public ResponseEntity<EntityModel<ProjektResource>> copyProjekt(
        ...
        return projektService.copyProjekt(projekt, screeningSheet.getBytes())
            .map(projektKopie -> ResponseEntity
                .created(mapper.createLink(ResourceMapper.REL_SELF, linkToCurrentMapping().toString(), ResourceMapper.PROJEKT, Map.of("projekt", projektKopie.getKuerzel())).toUri())
