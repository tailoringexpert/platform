...
@Data
public class KatalogAnforderung extends Anforderung implements Serializable {
    private static final long serialVersionUID = 1520118462633417279L;

    @Singular("identifikator")
    private Collection<Identifikator> identifikatoren;

    @Singular("phase")
    private Collection<Phase> phasen;

    @Builder
    public KatalogAnforderung(
        String text,
        String position,
        Collection<DRD> drds,
        Collection<Phase> phasen,
        Collection<Identifikator> identifikatoren,
        Referenz referenz) {
